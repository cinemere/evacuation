...
class GameEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        self.obstacle_size = np.array([1.0, 0.25])
        self.obstacle_line = np.array([[self.obstacle_pos[0] + 0.05 * self.obstacle_size[0], self.obstacle_pos[1] + 0.5 * self.obstacle_size[1]],
                                       [self.obstacle_pos[0] + 0.95 * self.obstacle_size[0], self.obstacle_pos[1] + 0.5 * self.obstacle_size[1]]])
        ...
        self.prev_pedestrians_position = None
    ...
    def plot_walk_diagram(self, path='model/images/', n_episode=0):
        ...
        obstacle = mpatches.Rectangle((self.obstacle_pos), self.obstacle_size[0], self.obstacle_size[1], color = 'red', alpha=0.1)
        ...
        for pedestrian_id in range(self.N_pedestrians):
            ...
            ax.plot(states[:, pedestrian_id + 1, 0], states[:, pedestrian_id + 1, 1], alpha=0.2, marker='.', linewidth=3, color=color)
    ...
    def _move(self, action):
        ...
        self.prev_pedestrians_position = self.pedestrians_position.copy()
        ...
        # self.agent_direction = (np.random.rand(2) - 0.5) * 2 * self.step_size  # only for random trajectories!!!!

        if not self._is_wall_collision(self.agent_position + self.agent_direction) and not self._is_agent_obstacle_collision():
        ...
        self._obstacle_processing()

        self._status(reward_on=True)

    def _orientation(self, a, b, c): #p a,  q b,  r c
    ...
    def _intersection_of_line_segments(self, p1, q1, line):
    ...
    def _is_agent_obstacle_collision(self, pos = None, size = None):

        if (pos is None) and (size is None):
            pos, size = self.obstacle_pos, self.obstacle_size

        curr = self.agent_position + self.agent_direction
        
        if (pos[0] < curr[0] and curr[0] < (pos[0] + size[0]) and
            pos[1] < curr[1] and curr[1] < (pos[1] + size[1])):
    ...
    def _obstacle_processing(self, pos = None, size = None):

        if (pos is None) and (size is None):
            pos, size = self.obstacle_pos, self.obstacle_size
        
        x_intersection = np.logical_and(pos[0] < self.pedestrians_position[:, 0], self.pedestrians_position[:, 0] < (pos[0] + size[0]))
        y_intersection = np.logical_and(pos[1] < self.pedestrians_position[:, 1], self.pedestrians_position[:, 1] < (pos[1] + size[1]))

        idx = np.arange(self.N_pedestrians)[np.logical_and(x_intersection, y_intersection)]

        left_lower_x, left_lower_y = pos[0], pos[1]
        left_upper_x, left_upper_y = pos[0], pos[1] + size[1]
        right_lower_x, right_lower_y = pos[0] + size[0], pos[1]
        right_upper_x, right_upper_y = pos[0] + size[0], pos[1] + size[1]

        upper_line = np.array([[left_upper_x, left_upper_y], [right_upper_x, right_upper_y]])
        lower_line = np.array([[left_lower_x, left_lower_y], [right_lower_x, right_lower_y]])
        left_line = np.array([[left_lower_x, left_lower_y], [left_upper_x, left_upper_y]])
        right_line = np.array([[right_lower_x, right_lower_y], [right_upper_x, right_upper_y]])

        if len(idx) > 0:
            for id in idx:
                curr = self.pedestrians_position[id]
                # prev = self.pedestrians_position[id] - self.pedestrians_direction[id]
                prev = self.prev_pedestrians_position[id]
                
                # upper line intersection
                if self._intersection_of_line_segments(curr, prev, upper_line):
                    # print('hi! -> up')
                    # we need to perform reflection upper the line
                    self.pedestrians_position[id][1] += 2 * abs(pos[1] + size[1] - curr[1])
                    self.pedestrians_direction[id][1] += 2 * abs(pos[1] + size[1] - curr[1])

                # lower line intersection
                elif self._intersection_of_line_segments(curr, prev, lower_line):
                    # print('hi! -> down')
                    # we need to perform reflection lower the line
                    self.pedestrians_position[id][1] -= 2 * abs(curr[1] - pos[1])
                    self.pedestrians_direction[id][1] -= 2 * abs(curr[1] - pos[1])
                
                # left line intersection
                if self._intersection_of_line_segments(curr, prev, left_line):
                    # print('hi! -> left')
                    # we need to perform reflection left the line
                    self.pedestrians_position[id][0] -= 2 * abs(curr[0] - pos[0])
                    self.pedestrians_direction[id][0] -= 2 * abs(curr[0] - pos[0])

                # right line intersection
                elif self._intersection_of_line_segments(curr, prev, right_line):
                    # print('hi! -> right')
                    # we need to perform reflection right the line
                    self.pedestrians_position[id][0] += 2 * abs(pos[0] + size[0] - curr[0])
                    self.pedestrians_direction[id][0] += 2 * abs(pos[0] + size[0] - curr[0])
