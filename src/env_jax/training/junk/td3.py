...
def make_env(seed, idx):
    def thunk():
        ...
        # Wrappers
        # env = GymAutoResetWrapper(env)  # TODO add
        env = GravityEncoding(env)
        env_params = env.default_params(**dataclasses.asdict(env_params))
        
        # Seeding
        rng = jax.random.key(seed + idx)
        
        return rng, env, env_params
    return thunk
# %%
...
# env setup
envs = gym.vector.SyncVectorEnv([make_env(args.seed, 0)])
