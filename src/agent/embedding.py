...
class NNSetEmbedding(nn.Module):
    def __init__(self, input_size):
        ...
        self.hidden_size = int(input_size / 5) * 2
...
if __name__ =='__main__':
    ...
    obs_batch = np.random.rand(4, 25)  # 2 + 4 + 4 = 10
    obs_batch = 2 * (obs_batch - 0.5)
    obs_batch = torch.tensor(obs_batch)
    print('initial random tensor of state')
    print(obs_batch)

    emb = NNSetEmbedding(25)
    print('its embedding')
    print(emb(obs_batch))